...
ROWS = [
    ...
    ("kv", "Name", "Prathamesh More"),
    ("kv", "Role", "Full Stack Developer"),
    ("kv", "Focus", "AI • Cybersecurity"),
    ...
    ("kv", "Languages", "Python • TypeScript • JavaScript • C++"),
    ("kv", "Database", "MongoDB • PostgreSQL • Supabase"),
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">prathamesh@github: ~$ neofetch</text>')
y = TITLEBAR_H + 30
for i, row in enumerate(ROWS):
    ...
    if kind == "gap":
        ...
        continue
    if kind == "host":
        line_start = KEY_X + 185
        inner = (
            f'<text x="{KEY_X}" y="{y:.1f}" font-size="14" font-weight="700">'
            f'<tspan fill="{GREEN}">prathamesh</tspan>'
            f'<tspan fill="{MUTED}">@</tspan>'
            f'<tspan fill="{ACCENT}">github</tspan>'
            f'</text>'
            f'<line x1="{line_start}" y1="{y-4:.1f}" x2="{W-PAD}" y2="{y-4:.1f}" '
